...
"""
Mission application that sets the power state of the ADCS module ON.
"""
...
def main():
    ...
    logger = app_api.logging_setup("set-adcs-on")
...
def on_command(logger, SERVICES):
    ...
    request = '''
    mutation {
        controlPower(controlPowerInput: {power: ON}) {
            success
            errors
        }
    }
    ...
    # check results
    if success:
        logger.info("Set ADCS power ON.")
    else:
        logger.warn("Unable to set ADCS power ON: {}.".format(errors))
