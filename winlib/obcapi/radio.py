...
class Radio:
    ...
    def __init__(self):
        ...
        self.logger = app_api.logging_setup("radio-service")
        self.use_uart = False
        try: 
        ...
        if self.write(command):
            for i in range(config.RETRY):
                ...
                if response == config.RETURN_CODE[0]:
                    self.logger.info("Got back {} message from radio sending command: {}.".format(response, command))
                    return True
                elif response == config.RETURN_CODE[1]:
                    self.logger.warn("Got back {} message from radio sending command: {}.".format(response, command))
                    return False
            return False
        else:
    ...
    def read_result(self, expected_response):
        for i in range(config.RETRY):
            ...
            if response == expected_response:
                self.logger.info("Got back successful {} message from radio.".format(response))
                return True
            elif response == config.RETURN_CODE[1]:
                self.logger.warn("Got back {} message from radio sending command.".format(response))
                return False
            else:
                self.logger.warn("Got back {} message from radio but expecting: {}.".format(response, expected_response))
        return False
...
######## QUERIES AND MUTATIONS ################
    def ping(self):
        # should send a ping to test connection to ground station
        try:
            command = "ping"
            response = "pong"
            if self.write(command):
                ...
                return True, []
                #else:
                #    return False, ["Radio did not send back sucessful result from {}".format(command)]
            return False, ["Could not send radio message: {}".format(command)]
            ...
            if self.send_command(command):
                ...
                if self.read_image("/home/kubos/images/image.jpg"):
                    if self.read_result(response):
                        ...
                        return False, ["Radio did not send back successful result from {}".format(command)]
                else:
                    return False, ["Error trying to read image from Radio."]
            return False, ["Could not send radio successful {}".format(command)]
