...
"""
Mission application that turns on a output port 2 on EPS module.
"""
...
def main():
    ...
    logger = app_api.logging_setup("turn-port-on-2")
...
def on_command(logger, SERVICES):
    ...
    # check results
    if success:
        logger.info("Turned on port 2.")
    else:
        logger.warn("Uable to turn on port 2: {}.".format(errors))
